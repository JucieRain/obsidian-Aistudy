# Wiki 操作日志

> 记录所有知识库操作的时间线

---

## [2026-04-22] init | 知识库初始化

**操作类型**：系统初始化
**执行内容**：
- 创建目录结构：wiki/concepts, wiki/tools, wiki/people, wiki/trends, wiki/practices, wiki/synthesis
- 创建初始文件：wiki/index.md, wiki/log.md
- 编写规则文档：CLAUDE.md
- 整合素材目录：raw → Clippings/papers

**当前状态**：
- 目录结构完成 ✓
- 规则文档完成 ✓
- 待录入素材：13 个

---

## 待处理素材清单

| 序号 | 素材路径 | 类型 | 状态 |
|-----|---------|------|------|
| 1 | Clippings/articles/Claude Code 国内丝滑部署指南.md | article | 待处理 |
| 2 | Clippings/articles/Hermes Agent 新手教程.md | article | 待处理 |
| 3 | Clippings/articles/Obsidian + Claude Code 教程.md | article | 待处理 |
| 4 | Clippings/articles/OpenClaw集成飞书.md | article | 待处理 |
| 5 | Clippings/articles/飞书云文档.md | article | 待处理 |
| 6 | Clippings/articles/一定要用 Claude 管理 Obsidian.md | article | 待处理 |
| 7 | Clippings/articles/优质AI信息源推荐.md | article | 待处理 |
| 8 | Clippings/articles/使用git实现obsidian同步.md | article | 待处理 |
| 9 | Clippings/articles/Hermes + 飞书踩坑修复.md | article | 待处理 |
| 10 | Clippings/articles/用Karpathy 方法改造知识系统.md | article | 待处理 |
| 11 | Clippings/papers/OpenClaw 完全指南.pdf | paper | 待处理 |
| 12 | Clippings/papers/OpenClaw橙皮书.pdf | paper | 待处理 |
| 13 | Clippings/papers/OpenClaw蓝皮书.pdf | paper | 待处理 |

---

*日志格式：## [日期] 操作类型 | 标题*
*操作类型：ingest | lint | query | init*