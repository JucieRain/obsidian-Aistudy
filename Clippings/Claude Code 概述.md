...
published:
created: 2026-04-22
description: "Claude Code 是一个代理编码工具，可以读取你的代码库、编辑文件、运行命令，并与你的开发工具集成。可在终端、IDE、桌面应用和浏览器中使用。"
tags:
  - "clippings"
---
...
首次使用时，系统会提示你登录。就这样！ [继续快速入门 →](https://code.claude.com/docs/zh-CN/quickstart)

查看 [高级设置](https://code.claude.com/docs/zh-CN/setup) 了解安装选项、手动更新或卸载说明。如果遇到问题，请访问 [故障排除](https://code.claude.com/docs/zh-CN/troubleshooting) 。
...
[`CLAUDE.md`](https://code.claude.com/docs/zh-CN/memory) 是一个 markdown 文件，你可以将其添加到项目根目录，Claude Code 会在每个会话开始时读取它。使用它来设置编码标准、架构决策、首选库和审查清单。Claude 还会在工作时构建 [自动内存](https://code.claude.com/docs/zh-CN/memory#auto-memory) ，保存学习内容，如构建命令和调试见解，跨会话使用，无需你编写任何内容。

创建 [自定义命令](https://code.claude.com/docs/zh-CN/skills) 来打包你的团队可以共享的可重复工作流，如 `/review-pr` 或 `/deploy-staging` 。

[Hooks](https://code.claude.com/docs/zh-CN/hooks) 让你在 Claude Code 操作之前或之后运行 shell 命令，如在每次文件编辑后自动格式化或在提交前运行 lint。

运行代理团队并构建自定义代理

生成 [多个 Claude Code 代理](https://code.claude.com/docs/zh-CN/sub-agents) ，同时处理任务的不同部分。主导代理协调工作、分配子任务并合并结果。
...
- [云计划任务](https://code.claude.com/docs/zh-CN/web-scheduled-tasks) 在 Anthropic 管理的基础设施上运行，因此即使你的计算机关闭，它们也会继续运行。从网络、桌面应用或通过在 CLI 中运行 `/schedule` 来创建它们。
...
- 向 [Dispatch](https://code.claude.com/docs/zh-CN/desktop#sessions-from-dispatch) 发送来自你手机的任务，并打开它创建的桌面会话
...
除了上面的 [终端](https://code.claude.com/docs/zh-CN/quickstart) 、 [VS Code](https://code.claude.com/docs/zh-CN/vs-code) 、 [JetBrains](https://code.claude.com/docs/zh-CN/jetbrains) 、 [桌面](https://code.claude.com/docs/zh-CN/desktop) 和 [网络](https://code.claude.com/docs/zh-CN/claude-code-on-the-web) 环境外，Claude Code 还与 CI/CD、聊天和浏览器工作流集成：

| 我想要… | 最佳选项 |
| --- | --- |
| 从我的手机或另一台设备继续本地会话 | [远程控制](https://code.claude.com/docs/zh-CN/remote-control) |
| 从 Telegram、Discord 或我自己的 webhook 推送事件到会话中 | [Channels](https://code.claude.com/docs/zh-CN/channels) |
| 在本地启动任务，在移动设备上继续 | [网络](https://code.claude.com/docs/zh-CN/claude-code-on-the-web) 或 [Claude iOS 应用](https://apps.apple.com/app/claude-by-anthropic/id6473753684) |
| 按定期计划运行 Claude | [云计划任务](https://code.claude.com/docs/zh-CN/web-scheduled-tasks) 或 [桌面计划任务](https://code.claude.com/docs/zh-CN/desktop#schedule-recurring-tasks) |
| 自动化 PR 审查和问题分类 | [GitHub Actions](https://code.claude.com/docs/zh-CN/github-actions) 或 [GitLab CI/CD](https://code.claude.com/docs/zh-CN/gitlab-ci-cd) |
| 在每个 PR 上获得自动代码审查 | [GitHub Code Review](https://code.claude.com/docs/zh-CN/code-review) |
| 将 Slack 中的错误报告路由到拉取请求 | [Slack](https://code.claude.com/docs/zh-CN/slack) |
| 调试实时网络应用 | [Chrome](https://code.claude.com/docs/zh-CN/chrome) |
...
- [快速入门](https://code.claude.com/docs/zh-CN/quickstart) ：通过你的第一个真实任务，从探索代码库到提交修复
- [存储说明和内存](https://code.claude.com/docs/zh-CN/memory) ：使用 CLAUDE.md 文件和自动内存为 Claude 提供持久说明
...
- [设置](https://code.claude.com/docs/zh-CN/settings) ：为你的工作流自定义 Claude Code
- [故障排除](https://code.claude.com/docs/zh-CN/troubleshooting) ：常见问题的解决方案
