...
**优先使用搜索工具而非遍历 index.md**：

当 wiki 页面较多时，使用搜索脚本定位相关页面：
```bash
.claude/wiki-search.sh <关键词> [限制数量]
```
...
# 原始素材中（反向链接）
在素材文件的 frontmatter 或末尾添加：
referenced_by: [[wiki/concepts/某概念.md]]
```

**双向链接目的**：
- wiki → 素材：快速追溯到原文验证
- 素材 → wiki：素材更新时能提醒相关 wiki 页面需同步

### 必须标注的内容

| 类型 | 示例 |
|-----|------|
| 数字/百分比 | "47.2%" → 标注行号 + 双向链接 |
| 具体结论 | "该公司是行业第一" → 标注行号 + 双向链接 |
| 人物观点 | "Karpathy认为..." → 标注行号 + 双向链接 |
| 时间/日期 | "2026年4月发布" → 标注行号 + 双向链接 |
...
录入素材时：
1. 读取素材全文，记录关键事实所在行号范围
2. 写入 wiki 页面时，每个关键事实添加来源标注（使用 wikilink）
3. 在素材文件中添加 `referenced_by` 字段指向 wiki 页面
...
2. 对比当前日期与 `next_due`
3. 如果超期 → 提醒用户：
   - "距上次健康检查已 X 天，建议执行 lint"
   - "距上次深度体检已 X 天，建议执行深度 lint"
